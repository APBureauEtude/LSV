...
function renderCellWithVariable(field, variables, projectId) {
    ...
    // Handle object (value or variable)
    if (typeof field === 'object') {
        if (field.type === 'variable') {
            let varValue = 0;
            if (variables[field.name] && variables[field.name].declaration) {
                varValue = variables[field.name].declaration.value || 0;
            }
            
            let badgeClass = field.isDeclaration ? 'var-declaration' : 'var-call';
            let varType = getVariableType(field.name);
            
            // Always show badge, show value only if not 0
            let valueDisplay = varValue !== 0 ? formatNumber(varValue) + ' ' : '';
            return `${valueDisplay}<span class="variable-badge ${badgeClass}" data-var-name="${field.name}" data-var-type="${varType}" data-is-declaration="${field.isDeclaration}" data-project-id="${projectId}">${field.name}</span>`;
...
function renderMetreTable(projectId) {
    ...
    let html = `
        <table class="metre-table">
            <thead>
                <tr>
                    <th width="40" style="background: #f0f0f0; color: #666;">#</th>
                    <th width="60">Code</th>
                    <th width="180">Désignation</th>
                    <th width="40">N</th>
                    <th width="40">Op</th>
                    <th width="60">L</th>
                    <th width="70">Total L</th>
                    <th width="50">l</th>
                    <th width="50">h</th>
                    <th width="50">Ens.</th>
                    <th width="70" class="col-plus">Val (+)</th>
                    <th width="70" class="col-moins">Val (-)</th>
                    <th width="50">Unit</th>
                    <th width="70">Qté T.</th>
                    <th width="60">PU</th>
                    <th width="80">Total HT</th>
                </tr>
            </thead>
            <tbody>`;
        ...
        if (block.type === 'file') {
            ...
            if (block.data && block.data.length > 0) {
                block.data.forEach((row, i) => {
                    ...
                    let designationDisplay = row.designation || '';
                    if (isSubtotal) {
                        designationDisplay = '<div style="display: flex; justify-content: space-between; align-items: center;"><span>' + (row.designation || '') + '</span><span style="font-size: 10px; color: #7f8c8d; font-style: italic;">Sous-Total</span></div>';
                    }
                    ...
                    if (isSubtotal) {
                        ...
                        totalLCellHtml = `<td class="col-calculated clickable" data-field="totall" data-row="${i}" style="${borderTopOnly}">${totalLDisplay}</td>`;
                    }
                    ...
                    html += `<tr class="${rowClass}" data-row="${i}" data-block-id="${block.id}" data-block-index="${blockIndex}" data-project-id="${projectId}" data-is-subtotal="${isSubtotal}" style="${rowStyle}">
                        <td style="background: #f0f0f0; color: #999; text-align: center; font-size: 11px; ${lineNumBorder}">${lineNumber}</td>
                        <td class="text-left ${isSubtotal ? '' : 'editable'}" data-field="code" style="${borderLeft}">${row.code || ''}</td>
                        <td class="text-left ${isSubtotal ? '' : 'editable'}" data-field="designation" style="${borderTopOnly}">${designationDisplay}</td>
                ...
                html += `
                    <tr class="block-table-footer" data-block-id="${block.id}" data-block-index="${blockIndex}" style="background: #f9f9f9; font-weight: bold;">
                        <td style="background: #f0f0f0; color: #999; text-align: center; font-size: 11px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding: 4px; position: relative;">
                            <div style="display: flex; align-items: center; justify-content: space-between; height: 100%;">
                                <span>${lineNumber}</span>
                                <button class="btn-delete-block" data-block-id="${block.id}" title="Supprimer ce bloc" style="background: white; border: 1px solid #666; border-radius: 50%; cursor: pointer; color: #333; font-size: 10px; width: 16px; height: 16px; padding: 0; opacity: 0.5; display: flex; align-items: center; justify-content: center; line-height: 1; flex-shrink: 0;">−</button>
                            </div>
                        </td>
                        <td data-field="code" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;"></td>
                        <td data-field="designation" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="n" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="op" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="l" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="totall" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="larg" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td data-field="h" style="border-bottom: 1px solid #bdc3c7;"></td>
                        <td class="editable-footer-ens" data-field="ens" data-block-id="${block.id}" style="text-align: center; border-bottom: 1px solid #bdc3c7; cursor: pointer; position: relative;">${block.footer.ens || ''}</td>
                        <td class="col-plus footer-calculated" data-field="valplus" style="border-bottom: 1px solid #bdc3c7; border-top: 1px solid #000; cursor: pointer;">${formatNumber(totalValPlus)}</td>
                        <td class="col-moins footer-calculated" data-field="valmoins" style="border-bottom: 1px solid #bdc3c7; border-top: 1px solid #000; cursor: pointer;">${formatNumber(-totalValMoins)}</td>
                        <td class="editable-footer" data-field="unit" data-block-id="${block.id}" style="border-bottom: 1px solid #bdc3c7; cursor: pointer;">${block.footer.unit || ''}</td>
                        <td class="footer-calculated" data-field="qtetotal" style="border-bottom: 1px solid #bdc3c7; cursor: pointer;">${formatNumber(footerQteTotal)}</td>
                        <td class="editable-footer" data-field="pu" data-block-id="${block.id}" style="border-bottom: 1px solid #bdc3c7; cursor: pointer;">${formatNumber(footerPU, 2)}</td>
                        <td class="footer-calculated" data-field="totalht" style="border-bottom: 1px solid #bdc3c7; border-right: 1px solid #bdc3c7; cursor: pointer;">${footerTotalHT !== 0 ? formatNumber(footerTotalHT, 2) + ' ' + appSettings.units.defaultCurrency : ''}</td>
                    </tr>`;
            } else {
                lineNumber++;
                // Empty table block
    ...
    $(document).off('keypress.celledit').on('keypress.celledit', function(e) {
        ...
        if (isSubtotal && field !== 'valplus' && field !== 'valmoins') return;
    ...
    // Context menu on Val+ or Val-
    $(`#workspace-${projectId} .metre-table td[data-field="valplus"], #workspace-${projectId} .metre-table td[data-field="valmoins"]`).on('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
        let rowIndex = parseInt($(this).data('row'));
        let pid = $(this).closest('tr').data('project-id');
        showValueSignContextMenu(pid, e, rowIndex);
        return false;
    });
    ...
    $(`#workspace-${projectId} .variable-badge`).on('click', function(e) {
        ...
        flashCellsByVariable(pid, varName);
    });
    ...
    // Context menu on variable badges in table
    $(`#workspace-${projectId} .variable-badge`).on('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
        let varName = $(this).data('var-name');
        let pid = $(this).data('project-id');
    ...
    // Context menu on Total L column
    $(`#workspace-${projectId} .metre-table td[data-field="totall"]`).on('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
        let rowIndex = parseInt($(this).data('row'));
        let pid = $(this).closest('tr').data('project-id');
        showCalculatedColumnContextMenu(pid, e, rowIndex, 'totall');
        return false;
    });
    
    // Context menu on Qté T. column
    $(`#workspace-${projectId} .metre-table td[data-field="qtetotal"]`).on('contextmenu', function(e) {
        e.preventDefault();
        e.stopPropagation();
        let rowIndex = parseInt($(this).data('row'));
        let pid = $(this).closest('tr').data('project-id');
        showCalculatedColumnContextMenu(pid, e, rowIndex, 'qtetotal');
        return false;
    });
    
    // Context menu on rows
    $(`#workspace-${projectId} .metre-table tr`).on('contextmenu', function(e) {
        // Don't show row context menu if clicking on Val+/Val-, Total L, or Qté T. cells
        let field = $(e.target).data('field');
        if (field === 'valplus' || field === 'valmoins' || field === 'totall' || field === 'qtetotal') {
            return;
        }
        e.preventDefault();
        let rowIndex = parseInt($(this).data('row'));
        ...
        // Only show context menu if it's a data row (has rowIndex)
        if (!isNaN(rowIndex) && blockId) {
        ...
        if (affectsTotalL && newValue !== '' && newValue !== '0') {
            ...
            if (totalL !== 0) {
                ...
                if (needsSubtotal) {
                    ...
                    // Create new empty row for continuation
                    let newEmptyRow = createEmptyRow();
                    
                    // Insert them after current row
                    block.data.splice(rowIndex + 1, 0, subtotalRow, newEmptyRow);
                    
                    console.log('[SOUS-TOTAL] Auto-created subtotal after row', rowIndex);
                    ...
                    setTimeout(() => {
                        ...
                        if ($ensCell.length > 0) {
                            ...
                            $(`#workspace-${projectId} .metre-table td`).removeClass('selected');
                            $ensCell.addClass('selected').focus();
                            updateSelection(projectId);
                        }
                    }, 60);
...
function showTableContextMenu(projectId, e, rowIndex, blockId) {
    ...
    let menuItems = [
        { label: '📞 Appel Variable ▶', action: () => {}, hasSubmenu: true, projectId: projectId, rowIndex: rowIndex, blockId: blockId },
        { separator: true },
        { label: '📏 Variable L', action: () => assignVariable(projectId, rowIndex, 'L', blockId) },
        { label: '📐 Variable S', action: () => assignVariable(projectId, rowIndex, 'S', blockId) },
        { label: '📦 Variable V', action: () => assignVariable(projectId, rowIndex, 'V', blockId) },
        { separator: true },
    ...
    menuItems.forEach(item => {
        if (item.separator) {
            ...
            if (item.hasSubmenu) {
                // Create submenu for variables
                menuItem.on('mouseenter', function() {
                    $('.context-submenu').remove();
                    showVariableCallSubmenu(item.projectId, item.rowIndex, $(this));
                });
...
function assignVariable(projectId, rowIndex, type) {
    let project = projects[projectId];
    if (!project.currentPoste) return;
    
    let metreData = project.currentPoste.data;
    let row = metreData[rowIndex];
    
    // Ask which field to apply the variable to
    let field = prompt(`Créer une variable ${type} dans quelle colonne ?\nOptions: n, l, larg, h, ens, pu, val+ (ou val-), qte`);
    if (!field) return;
    ...
    if (field === 'val+' || field === 'val-') {
        // For Val+/Val-, get the calculated value
        numValue = Math.abs(calculateValue(row, project.variables));
    } else if (field === 'qte') {
        // For Qté T., get the calculated total
        numValue = Math.abs(calculateValue(row, project.variables));
    } else {
        // For regular fields, get the current value
        let currentValue = row[internalField];
        numValue = getValue(currentValue, project.variables);
    }
    
    // Find next available number for this type
    let nextNum = getNextVariableName(projectId, type).substring(1); // Remove the letter
    let varName = type + nextNum;
    ...
    // Replace cell content with variable declaration
    row[internalField] = createVariableField(varName, true);
...
function getValue(field, variables) {
    if (!field) return 0;
    
    // If it's a simple number
    if (typeof field === 'number') return field;
    
    // If it's an object (variable or value wrapper)
    if (typeof field === 'object') {
        if (field.type === 'variable') {
            // Get value from variable declaration
            let varData = variables[field.name];
            if (varData && varData.declaration) {
                return varData.declaration.value || 0;
            }
            return 0;
        } else if (field.type === 'value') {
...
function renderVariables(projectId) {
    ...
    // Sort variables by type (L, S, V) and then by number
    let sortedVars = Object.entries(project.variables).sort((a, b) => {
        ...
        <div style="padding: 10px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                ...
                <div style="display:flex; gap:5px;">
                    <button class="tree-btn" onclick="sortVariables('${projectId}', 'asc')" title="Trier A→Z" style="font-size:10px; padding:2px 6px;">A→Z</button>
                    <button class="tree-btn" onclick="sortVariables('${projectId}', 'desc')" title="Trier Z→A" style="font-size:10px; padding:2px 6px;">Z→A</button>
                </div>
            </div>
    `;
    
    if (sortedVars.length === 0) {
        html += '<div style="padding:20px; text-align:center; color:#999; font-size:11px;">Aucune variable.<br>Tapez L1, S2, ou V3 dans une cellule pour créer une variable.</div>';
    } else {
        sortedVars.forEach(([varName, varData]) => {
            let varType = getVariableType(varName);
            let value = varData.declaration ? varData.declaration.value : 0;
            let callCount = varData.calls ? varData.calls.length : 0;
            let totalUsages = callCount + 1; // +1 for declaration
            
            html += `
                <div class="variable-item" data-var-name="${varName}" data-project-id="${projectId}" style="margin-bottom:8px; padding:6px; background:#f9f9f9; border-radius:4px; cursor:pointer;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span class="variable-badge var-declaration" data-var-name="${varName}" data-var-type="${varType}" data-project-id="${projectId}" style="cursor:pointer;">${varName}</span>
                        <span class="variable-value" style="font-weight:500; min-width:50px; font-size:11px;">${formatNumber(value)}</span>
                        <span class="variable-description" style="color:#666; font-size:10px; flex:1;">${varData.description}</span>
                        <span class="usage-count" style="color:#999; font-size:9px; white-space:nowrap;" title="${callCount} appel(s) + 1 déclaration">${totalUsages} usage(s)</span>
                        <button class="delete-var-btn" onclick="deleteVariableWithConfirmation('${projectId}', '${varName}')" title="Supprimer" style="background:none; border:none; cursor:pointer; font-size:14px; padding:2px; color:#e74c3c;">🗑️</button>
                    </div>
                </div>`;
        });
    }
    ...
    // Click on variable badge to flash cells in table
...
function renameVariableDescription(projectId, varName) {
    ...
    let newDesc = prompt('Nouvelle description pour ' + varName + ':', varData.description);
    if (newDesc !== null) {
        varData.description = newDesc.trim() || "Sans description";
        renderVariables(projectId);
    }
}
...
function saveSettings() {
    ...
    alert('Réglages enregistrés et appliqués !');
}
