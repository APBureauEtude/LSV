...
function showNewProjectDialog() {
    let html = `
        ...
        <div class="dialog-content" style="max-height: 70vh; overflow-y: auto;">
            ...
            <div class="form-group">
                <label>Ville :</label>
                <input type="text" id="inputVille" placeholder="Ville du chantier">
            </div>
            <div class="form-group">
                <label>Projet :</label>
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <div class="form-group" style="flex: 1;">
                    <label>Heure :</label>
                    <input type="time" id="inputHeureFin" value="18:00">
                </div>
            </div>
            <div style="display: flex; gap: 10px;">
                <div class="form-group" style="flex: 1;">
                    <label>Date RDV visite :</label>
                    <input type="date" id="inputDateRdv">
                </div>
                <div class="form-group" style="flex: 1;">
                    <label>Heure :</label>
                    <input type="time" id="inputHeureRdv" value="09:00">
                </div>
            </div>
            <div class="form-group">
                <label>Autre :</label>
...
function createNewProject() {
    ...
    let heureFin = $('#inputHeureFin').val();
    let dateRdv = $('#inputDateRdv').val();
    let heureRdv = $('#inputHeureRdv').val();
...
function showProjectInfo(projectId) {
    ...
    let html = `
        ...
        <div class="dialog-content" style="max-height: 70vh; overflow-y: auto;">
            ...
            <div class="form-group">
                <label>Ville :</label>
                <input type="text" id="editVille" value="${project.metadata.ville || ''}">
            </div>
            <div class="form-group">
                <label>Projet :</label>
            ...
            <div style="display: flex; gap: 10px;">
                ...
                <div class="form-group" style="flex: 1;">
                    <label>Heure :</label>
                    <input type="time" id="editHeureFin" value="${project.metadata.heureFin || '18:00'}">
                </div>
            </div>
            <div style="display: flex; gap: 10px;">
                <div class="form-group" style="flex: 1;">
                    <label>Date RDV visite :</label>
                    <input type="date" id="editDateRdv" value="${project.metadata.dateRdv || ''}">
                </div>
                <div class="form-group" style="flex: 1;">
                    <label>Heure :</label>
                    <input type="time" id="editHeureRdv" value="${project.metadata.heureRdv || '09:00'}">
                </div>
            </div>
            <div class="form-group">
                <label>Autre :</label>
...
function updateProjectInfo(projectId) {
    ...
    project.metadata.ville = $('#editVille').val().trim();
    ...
    project.metadata.heureFin = $('#editHeureFin').val();
    project.metadata.dateRdv = $('#editDateRdv').val();
    project.metadata.heureRdv = $('#editHeureRdv').val();
    ...
    layout.registerComponent('metre', function(container, componentState) {
        ...
        let metreHtml = `
            <div class="tree-container">
                ...
                <div class="zoom-area-${projectId}" style="flex:1; overflow:hidden; display:flex; flex-direction:column;"></div>
            </div>
        `;
...
function renderMetreTable(projectId) {
    ...
    // Colgroup pour les largeurs de colonnes (utilisé dans les deux tables)
    let colGroupHtml = `
        <colgroup>
            <col style="width:${colWidths.num}px;">
            <col style="width:${colWidths.code}px;">
            <col style="width:${colWidths.designation}px;">
            <col style="width:${colWidths.n}px;">
            <col style="width:${colWidths.op}px;">
            <col style="width:${colWidths.l}px;">
            <col style="width:${colWidths.totall}px;">
            <col style="width:${colWidths.larg}px;">
            <col style="width:${colWidths.h}px;">
            <col style="width:${colWidths.ens}px;">
            <col style="width:${colWidths.valplus}px;">
            <col style="width:${colWidths.valmoins}px;">
            <col style="width:${colWidths.unit}px;">
            <col style="width:${colWidths.qtetotal}px;">
            <col style="width:${colWidths.pu}px;">
            <col style="width:${colWidths.totalht}px;">
        </colgroup>`;
    
    // Header fixe séparé
    let headerHtml = `
        <div class="metre-header-wrapper" data-project-id="${projectId}">
            <table class="metre-table metre-header-table" data-project-id="${projectId}">
                ${colGroupHtml}
                <thead>
                    <tr class="metre-header-row">
                        <th data-col="num" style="background: #f0f0f0; color: #666; position: relative;">${colTitles.num}<div class="col-resizer" data-col="num"></div></th>
                        <th data-col="code" style="position: relative;">${colTitles.code}<div class="col-resizer" data-col="code"></div></th>
                        <th data-col="designation" style="position: relative;">${colTitles.designation}<div class="col-resizer" data-col="designation"></div></th>
                        <th data-col="n" style="position: relative;">${colTitles.n}<div class="col-resizer" data-col="n"></div></th>
                        <th data-col="op" style="position: relative;">${colTitles.op}<div class="col-resizer" data-col="op"></div></th>
                        <th data-col="l" style="position: relative;">${colTitles.l}<div class="col-resizer" data-col="l"></div></th>
                        <th data-col="totall" style="position: relative;">${colTitles.totall}<div class="col-resizer" data-col="totall"></div></th>
                        <th data-col="larg" style="position: relative;">${colTitles.larg}<div class="col-resizer" data-col="larg"></div></th>
                        <th data-col="h" style="position: relative;">${colTitles.h}<div class="col-resizer" data-col="h"></div></th>
                        <th data-col="ens" style="position: relative;">${colTitles.ens}<div class="col-resizer" data-col="ens"></div></th>
                        <th data-col="valplus" style="position: relative;" class="col-plus">${colTitles.valplus}<div class="col-resizer" data-col="valplus"></div></th>
                        <th data-col="valmoins" style="position: relative;" class="col-moins">${colTitles.valmoins}<div class="col-resizer" data-col="valmoins"></div></th>
                        <th data-col="unit" style="position: relative;">${colTitles.unit}<div class="col-resizer" data-col="unit"></div></th>
                        <th data-col="qtetotal" style="position: relative;">${colTitles.qtetotal}<div class="col-resizer" data-col="qtetotal"></div></th>
                        <th data-col="pu" style="position: relative;">${colTitles.pu}<div class="col-resizer" data-col="pu"></div></th>
                        <th data-col="totalht" style="position: relative;">${colTitles.totalht}<div class="col-resizer" data-col="totalht"></div></th>
                    </tr>
                </thead>
            </table>
        </div>`;
    ...
    // Corps du tableau
    let html = headerHtml + `
        <div class="metre-body-wrapper">
            <table class="metre-table metre-body-table" data-project-id="${projectId}">
                ${colGroupHtml}
                <tbody>`;
    ...
    html += `</tbody></table>
        <div class="metre-spacer" style="height: 150vh; min-height: 1200px;"></div>
    </div>`;
    
    // Le spacer est maintenant à l'intérieur du metre-body-wrapper
    ...
    // Synchroniser le scroll horizontal entre le header et le body
    $(`#workspace-${projectId} .metre-body-wrapper`).off('scroll.sync').on('scroll.sync', function() {
        let scrollLeft = $(this).scrollLeft();
        $(`#workspace-${projectId} .metre-header-wrapper`).scrollLeft(scrollLeft);
    });
    
    // Appliquer les formats de cellules sauvegardés
    applySavedCellFormats(projectId);
...
function initializeColumnResizing(projectId) {
    ...
    $(document).off('mouseup.colresize').on('mouseup.colresize', function(e) {
        ...
        // Appliquer la nouvelle largeur aux deux tables (header et body)
        $(`#workspace-${projectId} .metre-header-table th[data-col="${currentCol}"]`).css('width', newWidth + 'px');
        $(`#workspace-${projectId} .metre-body-table td[data-field="${currentCol}"]`).css('width', newWidth + 'px');
        $(`#workspace-${projectId} .col-resizer`).removeClass('resizing');
        ...
        // Re-rendre le tableau pour appliquer les largeurs correctement
        renderMetreTable(projectId);
...
function selectCell(projectId, cell) {
    ...
    // Mettre à jour la toolbar de formatage
    updateFormatToolbar();
}
...
document.getElementById('zoomSlider').addEventListener('input', function(e) {
    ...
    if (currentProjectId) {
        ...
        // Appliquer le zoom aux deux tables (header et body)
        $(`#workspace-${currentProjectId} .metre-header-table`).css('transform', `scale(${zoomFactor})`);
        $(`#workspace-${currentProjectId} .metre-header-table`).css('transform-origin', 'top left');
        $(`#workspace-${currentProjectId} .metre-body-table`).css('transform', `scale(${zoomFactor})`);
        $(`#workspace-${currentProjectId} .metre-body-table`).css('transform-origin', 'top left');
    }
});
...
function applyTableZoom(projectId) {
    ...
    // Appliquer le zoom aux deux tables (header et body)
    $(`#workspace-${projectId} .metre-header-table`).css('transform', `scale(${zoomFactor})`);
    $(`#workspace-${projectId} .metre-header-table`).css('transform-origin', 'top left');
    $(`#workspace-${projectId} .metre-body-table`).css('transform', `scale(${zoomFactor})`);
    $(`#workspace-${projectId} .metre-body-table`).css('transform-origin', 'top left');
...
function updateDaysRemaining() {
    ...
    // === DATE DE FIN ===
    let dateFinStr = project.metadata.endDate || project.metadata.dateFin;
    let heureFinStr = project.metadata.heureFin || '18:00';
    
    if (dateFinStr) {
        let endDate = new Date(dateFinStr + 'T' + heureFinStr);
        let now = new Date();
        ...
        if (diffMs < 0) {
            // En retard
            let retardDays = Math.abs(diffDays);
            let retardHours = Math.abs(diffHours);
            color = '#e74c3c';
            icon = '⚠️';
            if (retardDays > 0) {
                text = `${retardDays}j ${retardHours}h de retard`;
            } else {
                text = `${retardHours}h de retard`;
            }
            ...
            color = '#e67e22';
            icon = '⏰';
            text = `${diffDays}j ${diffHours}h restants`;
        } else {
            color = '#27ae60';
            icon = '📅';
            text = `${diffDays}j restants`;
        }
        
        displayParts.push(`<span style="color: ${color}; cursor: pointer;" title="Fin: ${dateFinStr} à ${heureFinStr}">${icon} ${text}</span>`);
    }
    
    // === DATE RDV ===
    let dateRdvStr = project.metadata.dateRdv;
    let heureRdvStr = project.metadata.heureRdv || '09:00';
    
    if (dateRdvStr) {
        let rdvDate = new Date(dateRdvStr + 'T' + heureRdvStr);
        let now = new Date();
        
        let diffMs = rdvDate - now;
        let diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
        let diffHours = Math.floor((diffMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        ...
        if (diffMs < 0) {
            // RDV passé - ne pas afficher ou afficher différemment
            color = '#999';
            icon = '✓';
            text = `RDV passé`;
        } else if (diffDays === 0 && diffHours <= 0) {
            ...
            color = '#9b59b6';
            icon = '🚨';
            text = `RDV dans ${diffHours}h`;
        } else if (diffDays === 1) {
            color = '#3498db';
            icon = '🗓️';
            text = `RDV demain à ${heureRdvStr}`;
        } else if (diffDays <= 7) {
            color = '#3498db';
            icon = '🗓️';
            text = `RDV dans ${diffDays}j ${diffHours}h`;
        } else {
            color = '#3498db';
            icon = '🗓️';
            text = `RDV dans ${diffDays}j`;
        }
        
        displayParts.push(`<span style="color: ${color}; cursor: pointer; margin-left: 15px;" title="RDV: ${dateRdvStr} à ${heureRdvStr}">${icon} ${text}</span>`);
    }
    
    if (displayParts.length === 0) {
        document.getElementById('days-remaining').innerHTML = '<span style="color: #999;">📅 Aucune date de fin</span>';
    } else {
        document.getElementById('days-remaining').innerHTML = displayParts.join('');
    }
...
// Variable pour sauvegarder les cellules sélectionnées (pour les color pickers)
let savedSelectedCells = [];

// Obtenir les cellules sélectionnées
function getSelectedCells() {
    if (!currentProjectId) return [];
    return $(`#workspace-${currentProjectId} .metre-table td.selected`).toArray();
}

// Sauvegarder la sélection actuelle (appelé avant d'ouvrir un color picker)
function saveSelection() {
    savedSelectedCells = getSelectedCells();
    console.log('[DEBUG] saveSelection:', savedSelectedCells.length, 'cellules sauvegardées');
}

// Ouvrir le color picker pour la couleur du texte
function openTextColorPicker() {
    saveSelection();
    document.getElementById('textColorPicker').click();
}

// Ouvrir le color picker pour la couleur de fond
function openBgColorPicker() {
    saveSelection();
    document.getElementById('bgColorPicker').click();
}

// Appliquer le formatage à une cellule
function applyCellFormat(cell, property, value) {
    let $cell = $(cell);
    let $row = $cell.closest('tr');
    let rowIndex = parseInt($row.data('row'));
    let blockId = $row.data('block-id');
    ...
    if (isNaN(rowIndex) || !blockId || !field) return;
    
    let project = projects[currentProjectId];
    if (!project || !project.currentPoste) return;
    
    let block = project.currentPoste.blocks.find(b => b.id === blockId);
    if (!block || !block.data || !block.data[rowIndex]) return;
    
    let row = block.data[rowIndex];
    
    // Initialiser le format de cellule si nécessaire
    if (!row._cellFormats) row._cellFormats = {};
    if (!row._cellFormats[field]) row._cellFormats[field] = {};
    
    row._cellFormats[field][property] = value;
}
...
function applyFontFamily(fontFamily) {
    ...
    cells.forEach(cell => {
        ...
        applyCellFormat(cell, 'fontFamily', fontFamily);
    });
}
...
function applyFontSize(size) {
    ...
    cells.forEach(cell => {
        $(cell).css('font-size', size + 'px');
        applyCellFormat(cell, 'fontSize', size);
    });
}

// Changer la taille de police (augmenter/diminuer)
function changeFontSize(delta) {
    ...
    let newSize;
    cells.forEach(cell => {
        let currentSize = parseInt($(cell).css('font-size')) || 11;
        newSize = Math.max(6, Math.min(72, currentSize + delta));
        $(cell).css('font-size', newSize + 'px');
        applyCellFormat(cell, 'fontSize', newSize);
    });
    
    // Toujours mettre à jour le sélecteur avec la taille de la première cellule
    if (cells.length > 0 && newSize) {
        $('#fontSize').val(newSize);
    }
}

// Basculer le formatage (gras, italique, etc.)
function toggleFormat(format) {
    let cells = getSelectedCells();
    if (cells.length === 0) return;
    
    let $btn;
    let cssProperty, cssValueOn, cssValueOff;
    ...
    switch (format) {
        ...
        case 'underline':
            ...
            cssValueOn = 'underline';
            cssValueOff = 'none';
            break;
        case 'strike':
            $btn = $('#btnStrike');
            cssProperty = 'text-decoration';
            cssValueOn = 'line-through';
            cssValueOff = 'none';
            break;
    }
    
    // Vérifier si le format est déjà actif sur la première cellule
    let firstCell = $(cells[0]);
    let currentValue = firstCell.css(cssProperty);
    
    // Détection améliorée pour font-weight (peut être "700", "bold", "400", "normal")
    let isActive = false;
    if (format === 'bold') {
        isActive = currentValue === 'bold' || currentValue === '700' || parseInt(currentValue) >= 600;
    } else if (format === 'italic') {
        isActive = currentValue === 'italic';
    } else {
        isActive = currentValue && currentValue.includes(cssValueOn);
    }
    
    let newValue = isActive ? cssValueOff : cssValueOn;
    
    cells.forEach(cell => {
        $(cell).css(cssProperty, newValue);
        applyCellFormat(cell, cssProperty, newValue);
    });
...
function applyTextColor(color) {
    console.log('[DEBUG] applyTextColor appelée avec:', color);
    
    // Utiliser la sélection sauvegardée si la sélection actuelle est vide
    let cells = getSelectedCells();
    console.log('[DEBUG] Cellules actuellement sélectionnées:', cells.length);
    
    if (cells.length === 0) {
        cells = savedSelectedCells;
        console.log('[DEBUG] Utilisation des cellules sauvegardées:', cells.length);
    }
    
    if (cells.length === 0) {
        console.log('[DEBUG] Aucune cellule à formater');
        return;
    }
    
    cells.forEach(cell => {
        console.log('[DEBUG] Application couleur texte sur cellule:', cell);
        cell.style.setProperty('color', color, 'important');
        applyCellFormat(cell, 'color', color);
    });
    
    // Mettre à jour la barre de couleur
    $('#textColorBar').css('background', color);
}

// Appliquer la couleur de fond
function applyBgColor(color) {
    console.log('[DEBUG] applyBgColor appelée avec:', color);
    ...
    cells.forEach(cell => {
        console.log('[DEBUG] Application couleur fond sur cellule:', cell);
        cell.style.setProperty('background-color', color, 'important');
        applyCellFormat(cell, 'backgroundColor', color);
    });
    
    // Mettre à jour la barre de couleur
    $('#bgColorBar').css('background', color);
}
...
function applyAlignment(align) {
    ...
    cells.forEach(cell => {
        ...
        applyCellFormat(cell, 'textAlign', align);
    });
    
    // Mettre à jour les boutons
    $('#btnAlignLeft, #btnAlignCenter, #btnAlignRight').removeClass('active');
    $(`#btnAlign${align.charAt(0).toUpperCase() + align.slice(1)}`).addClass('active');
}

// Appliquer les formats sauvegardés lors du rendu
function applySavedCellFormats(projectId) {
    let project = projects[projectId];
    if (!project || !project.currentPoste) return;
    ...
    project.currentPoste.blocks.forEach(block => {
        ...
        block.data.forEach((row, rowIndex) => {
            if (!row._cellFormats) return;
            
            Object.keys(row._cellFormats).forEach(field => {
                let formats = row._cellFormats[field];
                let $row = $(`#workspace-${projectId} .metre-table tr[data-block-id="${block.id}"][data-row="${rowIndex}"]`);
                let $cell = $row.find(`td[data-field="${field}"]`);
                
                if ($cell.length > 0) {
                    let cell = $cell[0];
                    Object.keys(formats).forEach(prop => {
                        // Utiliser setProperty avec important pour les couleurs
                        if (prop === 'color' || prop === 'backgroundColor') {
                            cell.style.setProperty(prop === 'backgroundColor' ? 'background-color' : prop, formats[prop], 'important');
                        } else {
                            $cell.css(prop, formats[prop]);
                        }
                    });
                }
            });
...
// Mettre à jour la toolbar selon la sélection
function updateFormatToolbar() {
    ...
    // Police
    let fontFamily = $cell.css('font-family').split(',')[0].replace(/['"]/g, '');
    $('#fontFamily').val(fontFamily);
    
    // Taille
    let fontSize = parseInt($cell.css('font-size'));
    $('#fontSize').val(fontSize);
    
    // Styles
    $('#btnBold').toggleClass('active', $cell.css('font-weight') === '700' || $cell.css('font-weight') === 'bold');
    $('#btnItalic').toggleClass('active', $cell.css('font-style') === 'italic');
    $('#btnUnderline').toggleClass('active', $cell.css('text-decoration').includes('underline'));
    $('#btnStrike').toggleClass('active', $cell.css('text-decoration').includes('line-through'));
    
    // Couleurs
    let textColor = $cell.css('color');
    let bgColor = $cell.css('background-color');
    
    // Alignement
    let textAlign = $cell.css('text-align');
    $('#btnAlignLeft, #btnAlignCenter, #btnAlignRight').removeClass('active');
    if (textAlign === 'left' || textAlign === 'start') $('#btnAlignLeft').addClass('active');
    else if (textAlign === 'center') $('#btnAlignCenter').addClass('active');
    else if (textAlign === 'right' || textAlign === 'end') $('#btnAlignRight').addClass('active');
}
...
document.addEventListener('keydown', function(e) {
    ...
    // Raccourcis de formatage (Ctrl+B, Ctrl+I, Ctrl+U)
...
// Initialisation des color pickers au chargement de la page
$(document).ready(function() {
    // Sauvegarder la sélection quand on clique sur un color picker
    $('#textColorPicker, #bgColorPicker').on('mousedown focus', function(e) {
        saveSelection();
        console.log('[COLOR] Sélection sauvegardée:', savedSelectedCells.length, 'cellules');
    });
    
    // Appliquer la couleur du texte
    $('#textColorPicker').on('input change', function() {
        let color = this.value;
        console.log('[COLOR] Couleur texte:', color, 'Cellules:', savedSelectedCells.length);
        
        if (savedSelectedCells.length === 0) {
            console.log('[COLOR] Pas de cellules sauvegardées');
            return;
        }
        
        savedSelectedCells.forEach(function(cell) {
            cell.style.setProperty('color', color, 'important');
            applyCellFormat(cell, 'color', color);
        });
    });
    ...
    $('#bgColorPicker').on('input change', function() {
        let color = this.value;
        console.log('[COLOR] Couleur fond:', color, 'Cellules:', savedSelectedCells.length);
