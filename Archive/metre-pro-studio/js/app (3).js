...
function changeFontSize(delta) {
    ...
    cells.forEach(cell => {
        ...
        let newSize = Math.max(6, Math.min(72, currentSize + delta));
    ...
    if (cells.length === 1) {
        let size = parseInt($(cells[0]).css('font-size'));
        $('#fontSize').val(size);
    }
}
...
function toggleFormat(format) {
    ...
    let isActive = currentValue === cssValueOn || currentValue.includes(cssValueOn.split('-')[0]);
